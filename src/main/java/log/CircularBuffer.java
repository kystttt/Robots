...
/**
 * Кольцевой буфер с потокобезопасными методами добавления в него, возвращения какого-то диапазона, возвращения
 * размера буфера. Также есть итератор. Когда буфер переполняются, перезаписывается самая последняя запись.
 *
